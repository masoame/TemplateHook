...
namespace Pipe
{
	...
	std::thread PipeIO::PipeInit([]
		{
			LogPipeH = CreateNamedPipeW(MsgPipeName, PIPE_ACCESS_OUTBOUND, PIPE_TYPE_BYTE, 1, 0, 0, 0, nullptr);
			if (LogPipeH)std::cout << "MsgPipe open success" << std::endl;
			...
			std::thread([]
				{
					...
					while (true)
					{
						if (ConnectNamedPipe(LogPipeH, nullptr))std::cout << "MsgPipe Client link success!!!" << std::endl;
